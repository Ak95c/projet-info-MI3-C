...
void run_teacher_mode(Quiz *newquiz) { // La fonction appelée par le menu principal
    ...
    int i;
    printf("Quand vous appuyez sur Entree, le texte est valide.\n");
    printf("Quel est le titre du quiz ? (sans espaces)\n");
    scanf(" %255[^\n]", newquiz->title);

    do {
        printf("Voulez vous qu'il y ait des points negatifs ? (1 = oui, 0 = non)\n");
        scanf("%d", &newquiz->negative_points);
    } while (newquiz->negative_points != 0 && newquiz->negative_points != 1);

    do {
        printf("Voulez vous que ce quiz soit sequentiel ? (1 = oui, 0 = non)\n");
        scanf("%d", &newquiz->sequential_mode);
    } while (newquiz->sequential_mode != 0 && newquiz->sequential_mode != 1);

    do {
        printf("Combien voulez vous de questions dans ce quiz ? (1-50)\n");
        scanf("%d", &newquiz->nb_questions);
    } while (newquiz->nb_questions <= 0 || newquiz->nb_questions > 50);

    do {
        printf("Voulez vous qu'il y ait des reponses multiples ? (1 = oui, 0 = non)\n");
    ...
    for (i = 0; i < newquiz->nb_questions; i++) {
        ...
        if (newquiz->multiple_answers == 0) {
            int bonne;
            do {
                printf("Quelle est la bonne reponse ? (1-4)\n");
                scanf("%d", &bonne);
            ...
            printf("Entrez les bonnes reponses (1 = correcte, 0 = incorrecte)\n");
            for (j = 0; j < 4; j++) {
                do {
    ...
    // Sauvegarde le quiz dans un fichier une fois toutes les questions saisies
    save_quiz_to_file(*newquiz);
}
